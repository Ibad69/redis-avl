...
int main(int argc, char **argv)
{
...
#if !defined(_MSC_VER)
    // On POSIX
    nob_cmd_append(&cmd, "cc", "-fsanitize=address", "-fstack-protector-all", "-Wall", "-Wextra", "-o", BUILD_FOLDER"prog", SRC_FOLDER"main.c");
    // nob_cmd_append(&cmd, SRC_FOLDER"server.c");
    // nob_cmd_append(&cmd, "-lcurl");
#else
    // On MSVC
    nob_cmd_append(&cmd, "cl", "-I.", "-o", BUILD_FOLDER"prog", SRC_FOLDER"main.c");
#endif // _MSC_VER
    ...
    // nob_cc(&cmd);
    // nob_cc_flags(&cmd);
    // nob_cc_output(&cmd, BUILD_FOLDER "prog");
    // nob_cc_inputs(&cmd, SRC_FOLDER"main.c");
    // // nob_cc_inputs(&cmd, SRC_FOLDER "hashmap.c");
    // if (!nob_cmd_run(&cmd)) return 1;
